...
export  const getCurrentUser = async (req,res)=>{
    try {
      ...
      const user = await User.findById(userId).populate("posts flick posts.author posts.comments saved saved.author story following")
      if(!user){
...
export const getProfile = async (req,res)=>{
    try {
        ...
        const user = await User.findOne({userName}).select("-password").populate("posts flick followers following")
...
export const getAllNotifications = async (req,res)=>{
    try {
        const notifications =  await Notification.find({
            receiver:req.userId
        }).populate("sender receiver post flick").sort({ createdAt: -1 });
...
export const markAsRead= async (req,res)=>{
    try {
       ...
       return res.status(200).json({
        ...
        return res.status(500).json({
            message:`read notifications error ${error}`
        })
    }
}