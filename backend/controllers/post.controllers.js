...
export const comment = async (req,res)=>{
 try {
    ...
    return res.status(500).json({
            message:`comments post error ${error}`
        })
    }
}
...
export const saved = async(req, res)=>{
     try {
        ...
        await user.populate({ path: "saved", populate: [{ path: "author", select: "name userName profileImage" }, { path: "comments.author", select: "name userName profileImage" }] });
        console.log(user.saved[0]);
        console.log(user.saved[0]?.author);
